...
import { verifyRestoreLock } from './restore-lock';
...
export class BackupService {
  ...
  async restoreBackup(id: string, options?: {
    ...
    /** Process-wide restore lock owner — only this token may clear maintenance or resume services */
    restoreLockToken?: string;
  }): Promise<{
    success: boolean;
    message: string;
    details?: any;
    ...
    try {
      ...
      if (!options?.restoreLockToken || verifyRestoreLock(options.restoreLockToken)) {
        try {
        ...
        console.warn('[Restore] Skipping service resume: restore lock not held by this caller');
      }
      ...
      const canClearMaintenance =
        !options?.restoreLockToken || verifyRestoreLock(options.restoreLockToken);

      if (canClearMaintenance) {
        try {
          const { MaintenanceCoordinator } = await import('./maintenance-coordinator');
          const maintenanceCoordinator = MaintenanceCoordinator.getInstance();
          await maintenanceCoordinator.resumeAllServices();
        ...
        console.warn('[Restore] Skipping maintenance clear and service resume in finally: restore lock not held by this caller');
      }
    }
  }
