...
router.get('/:id/reveal', ensureSuperAdmin, async (req, res) => {
  try {
    ...
    const apiKey = await aiCredentialsService.revealSystemCredential(credentialId);
    if (!apiKey) {
    ...
    console.error('Error revealing system AI credential:', error);
    res.status(500).json({
      success: false,
