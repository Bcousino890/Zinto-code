...
export function VideoNode({ data, isConnectable, id }: any) {
  ...
  return (
    <div
      className={`node-video p-3 rounded-lg bg-card border border-border shadow-sm relative group transition-[max-width,width] duration-200 ${
    ...
    >
      ...
      <div className="font-medium flex items-center gap-2 mb-2">
        <img 
          ...
          alt={t('flow_builder.video_upload_node_title', 'Video Message')} 
          className="h-4 w-4"
        />
      ...
      {isEditing ? (
        <div className="space-y-2">
          <MediaItemsEditor
            ...
            emptyLabel={t('flow_builder.media_items.empty_video', 'No videos added')}
            itemTypeLabel={t('flow_builder.video_label', 'Video')}
          />
      ...
      {/* Default output: next node after video send (when keyword routing is off). Keyword outputs use handles above. */}
      <Handle
        type="source"
        position={Position.Right}
