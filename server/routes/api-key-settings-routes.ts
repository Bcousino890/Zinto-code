  ...
  app.post('/api/settings/api-keys', ensureAuthenticated, async (req: any, res) => {
    try {
      ...
      const createdKey = await storage.createApiKey({
        ...
        isActive: true,
        rateLimitPerMinute: validatedConfiguration.rateLimitPerMinute ?? 300,
        rateLimitPerHour: validatedConfiguration.rateLimitPerHour ?? 5000,
        rateLimitPerDay: validatedConfiguration.rateLimitPerDay ?? 50000,
