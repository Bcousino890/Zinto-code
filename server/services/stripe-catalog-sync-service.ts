...
type StripeObject = { id: string; metadata?: Metadata; product?: string; coupon?: string; code?: string };
...
export type SyncAction = 'create_product' | 'update_product' | 'create_price' | 'archive_price' | 'create_coupon' | 'create_promotion_code' | 'archive_promotion_code' | 'create_plan_coupon' | 'archive_plan_coupon' | 'archive_product' | 'unchanged';
...
export class StripeCatalogSyncService {
  ...
  async syncPlan(planId: number, options: { dryRun?: boolean } = {}): Promise<SyncResult> {
    ...
    const fingerprint = catalogFingerprint(planFingerprintInput(plan, this.currency));
    const priceFingerprint = catalogFingerprint(planPriceFingerprintInput(plan, this.currency));
    const discountFingerprint = planDiscountIsActive(plan, this.now) ? catalogFingerprint(planDiscountFingerprintInput(plan, this.currency)) : undefined;
  ...
  async syncCoupon(couponId: number, options: { dryRun?: boolean } = {}): Promise<SyncResult> {
    ...
    const fingerprint = catalogFingerprint(couponFingerprintInput(coupon, this.currency));
    ...
    const active = couponIsRedeemable(coupon, this.now);
...
function planFingerprintInput(plan: Plan, currency: string) { return { name: plan.name, description: plan.description ?? null, isActive: plan.isActive !== false, price: planPriceFingerprintInput(plan, currency), discount: planDiscountFingerprintInput(plan, currency) }; }
...
function planDiscountInput(plan: Plan, currency: string, metadata: Metadata) { const input: Record<string, unknown> = { name: `${plan.name} plan discount`, metadata }; if (plan.discountType === 'percentage') input.percent_off = Number(plan.discountValue); else { input.amount_off = toMinorUnits(plan.discountValue, currency); input.currency = currency.toLowerCase(); } if (plan.discountEndDate) input.redeem_by = Math.floor(new Date(plan.discountEndDate).getTime() / 1000); return input; }
function couponFingerprintInput(coupon: Coupon, currency: string) { return { code: coupon.code, name: coupon.name, description: coupon.description ?? null, active: coupon.isActive !== false, start: timestamp(coupon.startDate), end: timestamp(coupon.endDate), discount: couponDiscountFingerprintInput(coupon, currency) }; }
