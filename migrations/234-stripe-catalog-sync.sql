...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'plans_stripe_sync_status_check'
  ) THEN
    ALTER TABLE plans
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'coupon_codes_stripe_sync_status_check'
  ) THEN
    ALTER TABLE coupon_codes
...
CREATE TABLE IF NOT EXISTS stripe_catalog_sync_jobs (
  ...
  CONSTRAINT stripe_catalog_sync_jobs_entity_fingerprint_unique
    UNIQUE (entity_type, entity_id, fingerprint)
);
