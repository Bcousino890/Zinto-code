...
import { MoreHorizontal, Edit2, Trash2, TrendingUp, DollarSign, Clock, Bot } from 'lucide-react';
import type { StageBotAssignmentSummary } from '@/hooks/use-pipeline';
import { PipelineStage, Deal, PipelineBoardStageSummary } from '@shared/schema';
...
interface StageHeaderProps {
  stage: PipelineStage;
  deals: Deal[];
  stageSummary?: PipelineBoardStageSummary;
  onEditStage: (stage: PipelineStage) => void;
  onDeleteStage: (stage: PipelineStage) => void;
  botAssignmentSummary?: StageBotAssignmentSummary;
  onAssignBot?: (stage: PipelineStage) => void;
}
...
export default function StageHeader({
  ...
  const stageStats = useMemo(() => {
    if (stageSummary) {
      return {
        count: stageSummary.totalCount,
        totalValue: stageSummary.totalValue,
        avgValue: stageSummary.avgValue,
        avgDaysInStage: stageSummary.avgDaysInStage,
        highPriority: stageSummary.highPriority,
        mediumPriority: stageSummary.mediumPriority,
        lowPriority: stageSummary.lowPriority,
      };
    }
      ...
      <div className="flex justify-between items-center mb-2">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className="text-[10px] text-muted-foreground truncate max-w-[140px]">
                    {botAssignmentSummary.flowName} · {botAssignmentSummary.channelName}
                  </span>
                </TooltipTrigger>
                <TooltipContent side="bottom">
                  <p className="text-xs">
                    {t('pipeline.stage_bot.assigned_tooltip', '{{flowName}} on {{channelName}}', {
                      flowName: botAssignmentSummary.flowName,
                      channelName: botAssignmentSummary.channelName,
                    })}
                  </p>
                </TooltipContent>
              </Tooltip>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            {onAssignBot && (
              <DropdownMenuItem onClick={() => onAssignBot(stage)}>
                <Bot className="mr-2 h-4 w-4" />
                {t('pipeline.stage_bot.assign_action', 'Assign Bot or Flow')}
              </DropdownMenuItem>
            )}
