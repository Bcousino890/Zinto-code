...
export function EditTwilioVoiceConnectionForm({ isOpen, onClose, onSuccess, connectionId }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await fetch(`/api/channel-connections/${connectionId}`, { credentials: 'include' });
        if (!response.ok) throw new Error('VOICE_LOAD_HTTP');
        const connection = await response.json();
        if (connection.channelType !== 'twilio_voice') throw new Error('VOICE_NOT_VOICE_CHANNEL');
        if (cancelled) return;
        setForm(parseVoiceConnectionFormData(connection.connectionData, connection.accountName || '', origin));
        setValidationReport(null);
      } catch (error: any) {
        if (!cancelled) {
          const code = error?.message;
          const description =
            code === 'VOICE_LOAD_HTTP'
              ? t('settings.voiceConnectionForm.errors.failedLoadVoiceChannel')
              : code === 'VOICE_NOT_VOICE_CHANNEL'
                ? t('settings.voiceConnectionForm.errors.notVoiceChannel')
                : error?.message || t('settings.voiceConnectionForm.errors.loadVoiceChannelGeneric');
          toastRef.current({
            title: t('settings.voiceConnectionForm.toast.loadFailedTitle'),
            description,
            variant: 'destructive',
          });
        }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        throw new Error(data.message || t('settings.voiceConnectionForm.errors.failedUpdateVoiceChannel'));
      }
      toast({
        title: t('settings.voiceConnectionForm.toast.updatedTitle'),
        description: t('settings.voiceConnectionForm.toast.updatedDescription', undefined, {
      ...
      toast({
        title: t('settings.voiceConnectionForm.toast.updateFailedTitle'),
        description: error.message || t('settings.voiceConnectionForm.toast.updateFailedDescription'),
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={open => !open && onClose()}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t('settings.voiceConnectionForm.edit.title')}</DialogTitle>
          <DialogDescription>{t('settings.voiceConnectionForm.edit.description')}</DialogDescription>
        </DialogHeader>
        <form className="space-y-6" onSubmit={handleSubmit}>
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="accountName">{t('settings.voiceConnectionForm.label.voiceChannelName')}</Label>
              <Input id="accountName" name="accountName" value={form.accountName} onChange={updateField} />
            </div>
            <div className="space-y-2">
