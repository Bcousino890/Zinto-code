  ...
  try {
    ...
    switch (paymentMethod) {
      ...
      case 'mercadopago':
        ...
        break;
      case 'paystack': {
        const payerEmail = req.user.email || company.companyEmail;
        if (!payerEmail) {
          return res.status(400).json({
            error: 'BILLING_EMAIL_REQUIRED',
            message: 'A valid billing email is required to complete Paystack checkout. Please add an email to your account or set a company billing email in settings.'
          });
        }
        paymentSession = await createPaystackPaymentSession(company, plan, enableAutoRenewal, originUrl, payerEmail);
        break;
      }
      case 'bank-transfer':
 ...
 */
async function createPaystackPaymentSession(company: any, plan: any, enableAutoRenewal: boolean = false, originUrl?: string, payerEmail?: string): Promise<{ id: string; url: string }> {
  ...
  const merchantCurrency = (paystackConfig.merchantCurrency as string)?.trim().toUpperCase() || appCurrency;

  if (!PAYSTACK_SUPPORTED_CURRENCIES.includes(merchantCurrency)) {
    throw new Error(`Paystack merchant currency "${merchantCurrency}" is not supported. Supported: ${PAYSTACK_SUPPORTED_CURRENCIES.join(', ')}`);
  }
  ...
  if (merchantCurrency === appCurrency) {
    ...
    try {
      ...
      throw new Error(`Cannot convert ${appCurrency} to ${merchantCurrency}. Set Paystack merchant currency to match your app currency (${appCurrency}), or enable ${appCurrency} in your Paystack dashboard.`);
    }
  }
  ...
  // Validate and use originUrl if provided, otherwise fallback to default URL
  let baseUrl = process.env.BASE_URL || 'http://localhost:5000';
  if (originUrl) {
    const validatedUrl = validateOriginUrl(originUrl);
    if (validatedUrl) {
  ...
  const initResponse = await fetch('https://api.paystack.co/transaction/initialize', {
    ...
    body: JSON.stringify({
      email: payerEmail || company.companyEmail,
  ...
  if (!initResponse.ok || !initData?.data?.authorization_url) {
    ...
    if (code === 'unsupported_currency' || (apiMessage && apiMessage.toLowerCase().includes('currency not supported'))) {
      throw new Error(`Currency ${paystackCurrency} is not enabled on your Paystack account. Go to Admin → Settings → Paystack and set "Merchant Currency" to the currency enabled on your integration (usually NGN for Nigeria, GHS for Ghana, ZAR for South Africa). Or contact Paystack support to enable ${paystackCurrency}.`);
    }
...
export function computeSubscriptionEndDate(
  plan: { billingInterval?: string | null; customDurationDays?: number | null },
  baseDate: Date
): Date {
  const newEndDate = new Date(baseDate);
  const billingInterval = plan.billingInterval || 'monthly';
  const customDurationDays = plan.customDurationDays;
...
export async function activateSubscriptionAfterPayment(
  ...
  try {
    ...
    const baseDate = (company.subscriptionEndDate && now <= company.subscriptionEndDate)
      ...
      : new Date(now);
    const newEndDate = computeSubscriptionEndDate(plan, baseDate);
