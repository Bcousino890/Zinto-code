...
export function getApiV2OpenApiDocument() {
  return {
    ...
    paths: {
      ...
      '/media': { get: { tags: ['Media'], summary: 'Descargar un archivo multimedia recibido', description: 'Requiere `media:read`. `type` y `filename` llegan ya resueltos en `data.media.url` del evento `message.received`; solo es accesible para la empresa dueña del mensaje.', security: [{ bearerAuth: [] }], parameters: [{ name: 'type', in: 'query', required: true, schema: { type: 'string', enum: ['image', 'video', 'audio', 'document'] } }, { name: 'filename', in: 'query', required: true, schema: { type: 'string' } }, integrationHeader], responses: { '200': { description: 'Contenido binario del archivo' }, ...errorResponses, '404': { description: 'Archivo inexistente o de otra empresa', content: json({ $ref: '#/components/schemas/Error' }) } } } },
