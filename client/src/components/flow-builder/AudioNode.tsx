...
export function AudioNode({ data, isConnectable, id }: any) {
  ...
  return (
    <div
      className={`node-audio p-3 rounded-lg bg-card border border-border shadow-sm relative group transition-[max-width,width] duration-200 ${
    ...
    >
      ...
      <div className="font-medium flex items-center gap-2 mb-2">
        <img 
          ...
          alt={t('flow_builder.audio_upload_node_title', 'Audio Message')} 
          className="h-4 w-4"
        />
      ...
      {isEditing ? (
        <div className="space-y-2">
          <MediaItemsEditor
            ...
            emptyLabel={t('flow_builder.media_items.empty_audio', 'No audio added')}
            itemTypeLabel={t('flow_builder.audio_label', 'Audio')}
          />
      ...
      {/* Default output: next node after audio send (when keyword routing is off). Keyword outputs use handles above. */}
      <Handle
        type="source"
        position={Position.Right}
