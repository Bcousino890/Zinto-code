...
interface ConversationContextProps {
  ...
  isLoadingInboxSettings: boolean;
...
function sortNonGroupConversationsNewestFirst(conversations: any[]): any[] {
...
export function ConversationProvider({ children }: ConversationProviderProps) {
  ...
  const totalUnreadCount = useMemo(() => {
    const individual = allConversations.reduce((sum, conv) => sum + (conv.unreadCount || 0), 0);
    const group = allGroupConversations.reduce((sum, conv) => sum + (conv.unreadCount || 0), 0);
    return individual + group;
  }, [allConversations, allGroupConversations]);
  ...
  const mergeContactIntoConversation = useCallback((conversation: any, updatedContact: any) => {
    if (!conversation || !updatedContact) return conversation;
    const conversationContactId = conversation.contact?.id ?? conversation.contactId;
    if (conversationContactId !== updatedContact.id) {
      return conversation;
    }
  ...
  const enrichTelegramListWithCachedLastMessage = useCallback(async (conversations: any[]) => {
    ...
    return Promise.all(
      conversations.map(async (conv: any) => {
        if (conv.channelType !== 'telegram' || conv.lastMessage) return conv;
        const cid = typeof conv.id === 'string' ? parseInt(conv.id, 10) : conv.id;
        if (!Number.isFinite(cid)) return conv;
        const latest = await messageCacheService.getLatestCachedMessageForPreview(cid);
        return latest ? { ...conv, lastMessage: latest } : conv;
      })
    );
  }, []);
  ...
  const fetchConversations = async (page: number = 1, append: boolean = false) => {
    ...
    try {
      ...
      try {
        conversations = await enrichTelegramListWithCachedLastMessage(conversations);
      } catch (e) {
        console.warn('[inbox] Could not hydrate Telegram last message from cache', e);
      }

      setAllConversations(prev => {
        const newConversations = append ? [...prev, ...conversations] : conversations;
  ...
  const fetchGroupConversations = async (page: number = 1, append: boolean = false) => {
    ...
    try {
      ...
      let groupConversations = data.conversations;
      try {
        groupConversations = await enrichTelegramListWithCachedLastMessage(groupConversations);
      } catch (e) {
        console.warn('[inbox] Could not hydrate Telegram last message from cache (groups)', e);
      }

      setAllGroupConversations(prev => {
        const newConversations = append ? [...prev, ...groupConversations] : groupConversations;
        return newConversations;
      });
  ...
  const { data: inboxSettings, isLoading: isLoadingInboxSettings } = useQuery<InboxSettings>({
  ...
  useEffect(() => {
    ...
    const selectedChannelIdRaw = localStorage.getItem('selectedChannelId');
    ...
    if (!(selectedContactId && (selectedChannelType || selectedChannelIdRaw))) {
      return;
    }
    ...
    const selectedChannelIdNum = selectedChannelIdRaw ? parseInt(selectedChannelIdRaw, 10) : null;
    const clearStorage = () => {
      localStorage.removeItem('selectedContactId');
      localStorage.removeItem('selectedChannelType');
      localStorage.removeItem('selectedChannelId');
    ...
    const whatsappTypes = ['whatsapp', 'whatsapp_unofficial', 'whatsapp_official'];
    const tryInitiateAndOpen = async (): Promise<void> => {
      if (selectedChannelIdNum == null || isNaN(selectedChannelIdNum)) {
        clearStorage();
        return;
      }
      const isWhatsApp = selectedChannelType != null && whatsappTypes.includes(selectedChannelType);
      if (!isWhatsApp) {
        clearStorage();
        return;
      }
      try {
        ...
        const initRes = await apiRequest('POST', '/api/conversations/whatsapp/initiate', {
          name: contact.name ?? '',
          phoneNumber: contact.phone,
          channelConnectionId: selectedChannelIdNum,
        });
        if (!initRes.ok) {
          clearStorage();
          return;
        ...
        const data = await initRes.json();
          if (data?.conversation?.id) {
            ...
            setActiveChannelId(selectedChannelIdNum);
            await refetchConversations();
          }
        } catch {
          // ignore; clear storage so we don't retry forever
        }
        clearStorage();
      };
    ...
    if (selectedChannelIdNum != null && !isNaN(selectedChannelIdNum)) {
        const channelIdMatch = conversations.find(conv =>
          conv.contactId === contactId && conv.channelId === selectedChannelIdNum
        );
        if (channelIdMatch) {
          setActiveConversationId(channelIdMatch.id);
          setActiveChannelId(channelIdMatch.channelId);
          clearStorage();
          return;
        }
      }
      ...
      tryInitiateAndOpen();
  }, [conversations, setActiveConversationId, setActiveChannelId, activeConversationId, conversationsPagination.loading, authLoading, user, areChannelTypesCompatible, refetchConversations]);
  ...
  const finalizeNonGroupConversationList = useCallback((list: any[]) => {
    const withContacts = list.map((c) =>
      !c?.isGroup && !c?.contact && c?.contactId ? enrichConversationWithContact(c) : c
    );
    return sortNonGroupConversationsNewestFirst(withContacts);
  }, [enrichConversationWithContact]);

  const refetchSingleConversation = useCallback(
    async (conversationId: number) => {
      try {
        const res = await apiRequest('GET', `/api/conversations/${conversationId}`);
        const fresh = await res.json();
        if (!fresh?.id || !canUserViewConversation(fresh)) {
          return;
        }
        setAllConversations(prev => {
          const exists = prev.some(c => c.id === fresh.id);
          const enriched = enrichConversationWithContact(fresh);
          if (!exists) {
            return finalizeNonGroupConversationList([enriched, ...prev]);
          }
          return finalizeNonGroupConversationList(
            prev.map(c => (c.id === fresh.id ? { ...c, ...enriched } : c))
          );
        });
      } catch {
        // Conversation may not be available yet (e.g. replication lag), or access denied.
      }
    },
    [canUserViewConversation, enrichConversationWithContact, finalizeNonGroupConversationList],
  );
  ...
  useEffect(() => {
    const justReconnected = isConnected && !wasSocketConnectedRef.current;
    ...
    if (isConnected) {
      ...
      // Refetch open thread from API — WebSocket events while disconnected are not replayed.
      if (justReconnected && activeConversationIdRef.current) {
        void fetchMessages(activeConversationIdRef.current, 1, false, true);
      }
    }
  ...
  const fetchMessages = async (
    ...
    try {
      ...
      // Page 1 loads always hit the API so messages persisted while the tab was closed are not hidden by IndexedDB.
      const shouldForceRefresh = forceRefresh || (page === 1 && !append);
      const result = await messageCache.loadMessages(conversationId, page, 10, isGroup, shouldForceRefresh);
  ...
  // If the conversation list says there is a newer message than the open thread, force an API refetch.
  const lastStaleRefetchRef = useRef<Record<number, number>>({});
  useEffect(() => {
    if (!activeConversationId || !user || authLoading) return;

    const conv =
      allConversations.find((c) => c.id === activeConversationId) ||
      allGroupConversations.find((c) => c.id === activeConversationId);
    if (!conv?.lastMessageAt) return;

    const thread = messages[activeConversationId] || [];
    if (thread.length === 0) return;

    const newestThreadTs = thread.reduce((best, m) => {
      const t = new Date(m.sentAt || m.createdAt || 0).getTime();
      return Number.isFinite(t) && t > best ? t : best;
    }, 0);

    const listTs = new Date(conv.lastMessageAt).getTime();
    const listHasUnknownLastMessage =
      !!conv.lastMessage?.id && !thread.some((m) => m.id === conv.lastMessage.id);

    if (!(listTs > newestThreadTs + 2000 || listHasUnknownLastMessage)) {
      return;
    }

    const lastRefetchAt = lastStaleRefetchRef.current[activeConversationId] || 0;
    if (Date.now() - lastRefetchAt < 5000) {
      return;
    }
    lastStaleRefetchRef.current[activeConversationId] = Date.now();
  ...
  useEffect(() => {
    const unsubscribe = onMessage('newMessage', (data) => {
      ...
      if (message && message.conversationId) {
        const previewTime = message.createdAt ?? message.sentAt;
        setAllConversations(prev => {
          ...
          const next = prev.map(conv =>
            conv.id === message.conversationId
              ? {
                  ...conv,
                  lastMessage: message,
                  lastMessageAt: previewTime ?? conv.lastMessageAt,
                }
              : conv
          );
          return finalizeNonGroupConversationList(next);
        });
        setAllGroupConversations(prev =>
    ...
    return unsubscribe;
  }, [onMessage, queryClient, browserNotifications, allConversations, allGroupConversations, finalizeNonGroupConversationList]);
  ...
  useEffect(() => {
    const unsubscribe = onMessage('messageUpdated', (data: any) => {
      const payload = data?.data;
      if (!payload?.messageId || !payload?.conversationId || !payload?.updates) {
        return;
      }
      const { messageId, conversationId, updates } = payload;
      ...
      setMessages(prev => {
        ...
        const nextList = [...list];
        nextList[idx] = { ...list[idx], ...updates };
        return { ...prev, [conversationId]: nextList };
      });

      messageCache.updateMessageInCache(messageId, updates).catch(console.error);

      const touchLast = (conv: any) => {
        if (!conv?.lastMessage || conv.lastMessage.id !== messageId) {
          return conv;
        }
        return { ...conv, lastMessage: { ...conv.lastMessage, ...updates } };
      };
      setAllConversations(prev => prev.map(touchLast));
      setAllGroupConversations(prev => prev.map(touchLast));
    });
    ...
    const unsubscribe = onMessage('conversationUpdated', (data) => {
      ...
      setAllConversations(prev => {
        ...
        if (
          ...
          !updatedConversation.isGroup &&
          !updatedConversation.groupJid
        ) {
          queueMicrotask(() => {
            void refetchSingleConversation(updatedConversation.id);
          });
        }
        const next = exists
          ? prev.map(conv =>
          ...
          : [enrichConversationWithContact(updatedConversation), ...prev];
        return finalizeNonGroupConversationList(next);
      });
    });

    return unsubscribe;
  }, [onMessage, showGroupChats, canUserViewConversation, enrichConversationWithContact, finalizeNonGroupConversationList, refetchSingleConversation]);
  ...
  useEffect(() => {
    const unsubscribe = onMessage('contactUpdated', (data) => {
      ...
      setAllConversations(prev => prev.map(conv => mergeContactIntoConversation(conv, updatedContact)));
      setAllGroupConversations(prev => prev.map(conv => mergeContactIntoConversation(conv, updatedContact)));
    });

    return unsubscribe;
  }, [onMessage, mergeContactIntoConversation]);
  ...
  useEffect(() => {
    const unsubscribe = onMessage('conversationCreated', (data) => {
      ...
      setAllConversations(prev => {
        ...
        return finalizeNonGroupConversationList([
          enrichConversationWithContact(newConversation),
          ...prev,
        ]);
      });
    ...
    return unsubscribe;
  }, [onMessage, showGroupChats, enrichConversationWithContact, finalizeNonGroupConversationList]);
  ...
  useEffect(() => {
    const unsubscribe = onMessage('newConversation', (data) => {
      ...
      setAllConversations(prev => {
        ...
        const next = exists
          ...
          : [enrichConversationWithContact(newConversation), ...prev];
        return finalizeNonGroupConversationList(next);
      });
    ...
    return unsubscribe;
  }, [onMessage, showGroupChats, queryClient, canUserViewConversation, enrichConversationWithContact, finalizeNonGroupConversationList]);
  ...
  useEffect(() => {
    const unsubscribe = onMessage('historySyncProgress', (data) => {
      ...
      // Show progress toast every 50 messages or at milestones
  ...
  const sendMediaMessage = async (
    ...
    if (typeof window !== 'undefined' && window.location?.origin) {
      xhrFormData.append('clientOrigin', window.location.origin);
    }

    const timeoutMs = options?.xhrTimeoutMs ?? getMediaUploadTimeoutMs(file.size);

    const response = await uploadFormDataWithProgress(
      `/api/conversations/${conversationId}/upload-media`,
      xhrFormData,
      {
    ...
    if (!response.ok) {
      try {
        ...
        throw new Error(
          errorData.message || errorData.error || t('inbox.failed_send_media', 'Failed to send media message'),
        );
      } catch (jsonError: any) {
        if (jsonError?.message && !jsonError.message.includes('Server error')) {
          throw jsonError;
        }
  ...
  const contextValue: ConversationContextProps = {
    ...
    inboxEmbeddingEnabled: inboxSettings?.inboxEmbeddingEnabled ?? true,
    transcriptionSettings: {
      enabled: inboxSettings?.audioTranscriptionEnabled ?? false,
      automatic: inboxSettings?.audioTranscriptionAutomatic ?? false,
      provider: inboxSettings?.audioTranscriptionProvider ?? 'openai',
      credentialSource: inboxSettings?.audioTranscriptionCredentialSource ?? 'auto'
    },
    imageAnalysisSettings: {
      enabled: inboxSettings?.imageAnalysisEnabled ?? false,
      provider: inboxSettings?.imageAnalysisProvider ?? 'openai',
      credentialSource: inboxSettings?.imageAnalysisCredentialSource ?? 'auto'
    },
    sendMessage,
    sendMediaMessage,
    loadMoreMessages,
