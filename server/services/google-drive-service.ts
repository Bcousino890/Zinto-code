...
export class GoogleDriveService {
  ...
  private async refreshAccessToken(): Promise<void> {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        console.log(`[GoogleDrive] Attempting to refresh access token (attempt ${attempt + 1}/${maxRetries})...`);
  ...
  async uploadBackup(filePath: string, filename: string): Promise<{ fileId: string; size: number }> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.log(`[Google Drive Upload] Starting upload of ${filename} (${(fileSize / 1024 / 1024).toFixed(2)} MB) - Attempt ${attempt}/${maxRetries}`);
        ...
        fileStream.on('data', (chunk) => {
          ...
          if (percent >= lastLoggedPercent + 10) {
            console.log(`[Google Drive Upload] Progress: ${percent}% (${(uploadedBytes / 1024 / 1024).toFixed(2)} MB / ${(fileSize / 1024 / 1024).toFixed(2)} MB)`);
        ...
        const response = await this.drive.files.create({
          ...
          onUploadProgress: (evt: any) => {
            if (evt.bytesRead) {
              ...
              if (percent >= lastLoggedPercent + 10) {
                console.log(`[Google Drive Upload] Upload progress: ${percent}% (${(evt.bytesRead / 1024 / 1024).toFixed(2)} MB)`);
                lastLoggedPercent = percent;
              }
            }
          }
