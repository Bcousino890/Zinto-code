                  ...
                  <div className="space-y-3 rounded-lg border p-3">
                    {selectedDesignerTable && layoutDrafts[selectedDesignerTable.id] ? (
                      <>
                        ...
                        <div><Label>{t('erp.restaurant.tableFloors.shape', 'Shape')}</Label><Select value={layoutDrafts[selectedDesignerTable.id].tableShape} onValueChange={(v) => setDraft(selectedDesignerTable.id, { tableShape: v as LayoutDraft['tableShape'] })} disabled={!canManage}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="rectangle">{t('erp.restaurant.tableFloors.shape_rectangle', 'Rectangle')}</SelectItem><SelectItem value="square">{t('erp.restaurant.tableFloors.shape_square', 'Square')}</SelectItem><SelectItem value="circle">{t('erp.restaurant.tableFloors.shape_circle', 'Circle')}</SelectItem></SelectContent></Select></div>
                        <div className="flex gap-2">
      ...
      <Dialog open={tableDialogOpen} onOpenChange={setTableDialogOpen}>
        <DialogContent className="sm:max-w-2xl">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><Label>{t('erp.restaurant.tableFloors.tableShape', 'Shape')}</Label><Select value={tableForm.tableShape} onValueChange={(v) => setTableForm((p) => ({ ...p, tableShape: v }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="rectangle">{t('erp.restaurant.tableFloors.shape_rectangle', 'Rectangle')}</SelectItem><SelectItem value="circle">{t('erp.restaurant.tableFloors.shape_circle', 'Circle')}</SelectItem><SelectItem value="square">{t('erp.restaurant.tableFloors.shape_square', 'Square')}</SelectItem></SelectContent></Select></div>
