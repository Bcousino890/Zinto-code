...
import { stopIntroTour } from "./tour";
export { startIntroTour, stopIntroTour } from "./tour";
...
type AppProps = {
  ...
  /**
   * When false, hides the odontogram language picker (top bar + Settings).
   * Useful when the host app owns locale selection. Default `true`.
   */
  showLanguagePicker?: boolean;
  /**
   * When false, hides the odontogram light/dark theme toggle (top bar + Settings).
   * Useful when the host app owns appearance. Default `true`.
   */
  showThemeToggle?: boolean;
  /**
   * When false, hides the top-bar green dot and title, but keeps the subtitle
   * (language / numbering / mode description). Useful when the host app
   * provides its own header chrome. Default `true`.
   */
  showBrand?: boolean;
  /**
   * Optional callback to request switching the host into edit mode. When
   * provided AND {@link readOnly} is true, a pencil button appears in the
   * top-bar actions; clicking it invokes this callback. Intended for hosts
   * that show a read-only chart (e.g. history view) and want a one-click
   * "edit the current live chart" affordance.
   */
  onRequestEdit?: () => void;
...
export default function App({
  ...
  // Apply `.dark` on `.odontogram-root` only — never mutate host `<html>`.
  ...
  const settingsState: SettingsState = {
    ...
    language: lang,
    onLanguage: setLang,
    showLanguage: showLanguagePicker,
    isDark,
  ...
  return (
    <div ref={themeRootRef} className={isDark ? "odontogram-root dark" : "odontogram-root"}>
      <header className="topbar">
        <div className="brand">
          {showBrand && <div className="dot"></div>}
          <div>
            {showBrand && <div className="title">{t("app.title")}</div>}
        ...
        <div className="topbar-actions">
          ...
          {onRequestEdit && (readOnlyProp ?? false) && (
            <div className="topbar-group">
              <button className="btn-theme" onClick={() => onRequestEdit()} title={t("topbar.edit")} aria-label={t("topbar.edit")}>
