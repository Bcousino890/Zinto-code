...
export default function KanbanBoard({ onAddDeal, activePipelineId }: KanbanBoardProps) {
  ...
  const handleLoadMoreDeals = useCallback(async (stageId: number) => {
    ...
    if (!pagination?.hasMore || loadingMoreStageId !== null) {
      return;
    }
  ...
  }, [stagePagination, stageSummaries, loadingMoreStageId, buildDealsQueryParams]);
  ...
  const handleDragEnd = (result: any) => {
    ...
    if (destination.droppableId === source.droppableId) {
      // In-stage reorder is not persisted; deals are ordered by lastActivityAt on the server.
      setOptimisticDeals({});
      return;
    }
