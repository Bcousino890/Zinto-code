...
/** Public host-integration API: serialize current odontogram state for persistence. */
export function collectOdontogramPayload(): Any {
  return collectExportPayload();
}

/** Public host-integration API: hydrate odontogram from a status JSON payload. */
export function importOdontogramPayload(data: Any): void {
  importStatus(data);
}
