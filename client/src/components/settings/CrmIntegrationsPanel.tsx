...
import { Check, Copy, Edit3, ExternalLink, Link2, Loader2, Plus, RefreshCw, ShieldCheck, XCircle } from 'lucide-react';
...
const FALLBACKS = {
  ...
  created: 'Integración creada', secret: 'Secreto del webhook', secretHelp: 'Guárdalo ahora. Por seguridad, no volverá a mostrarse.',
...
export function CrmIntegrationsPanel() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!editing && normalized?.webhookSecret) setCreatedSecret({ integration: normalized, secret: normalized.webhookSecret });
